...
export default function Timer({ onActivityCreated }: TimerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // FIXED: Properly format timestamps for Supabase
      const combinedStartTime = `${date}T${startTime}`;
      const combinedEndTime = `${date}T${endTime}`;
      ...
      console.log("Activity saved:", data); // Debug log
      ...
      if (onActivityCreated) {
        onActivityCreated(); // This should trigger a refresh in the parent component
      }
    } catch (error) {
